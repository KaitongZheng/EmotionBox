...
from data_label import Dataset_label
...
def get_options():
    ...
    parser.add_option('-s', '--session',
                      dest='sess_path',
                      type='string',
                      default='save/train_label.sess')

    parser.add_option('-d', '--dataset',
                      dest='data_path',
                      type='string',
                      default='dataset/processed/emotion_classical')
...
model_config = config.model_label ##change
...
def load_session():
    ...
    try:
        ...
        print('New session')
        sess_loaded = False
    model = PerformanceRNN_label(**model_config).to(device)
...
def load_dataset():
    global data_path
    dataset = Dataset_label(data_path, verbose=True)
...
try:
    ...
    for iteration, (events, labels) in enumerate(batch_gen):
        if use_transposition:
            offset = np.random.choice(np.arange(-6, 6))
            events, labels = utils.transposition(events, labels, offset)

        events = torch.LongTensor(events).to(device)  # 200X64(timestep * batch_size) input
        assert events.shape[0] == window_size

        if np.random.random() <= control_ratio:
            labels = torch.FloatTensor(labels).to(device)  # 200X64X24
            assert labels.shape[0] == window_size
        else:
            labels = None

        init = torch.randn(batch_size, model.init_dim).to(
            device)  # 64*32 to generate initial hidden layer 再训的效果不好可能是hidden又被初始化了
        outputs = model.generate(init, window_size, events=events[:-1], labels=labels,
                                 teacher_forcing_ratio=teacher_forcing_ratio,
                                 output_type='logit')  # 200X64X240,注意！！最后一个event没输入
